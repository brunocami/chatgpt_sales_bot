...
def embedding_info(mensaje):
    ...
    cursor.execute("SELECT url, contexto FROM informacion_delta")
    rows = cursor.fetchall()

    # Crear un DataFrame para almacenar los datos
    product_data_df = pd.DataFrame(rows, columns=['url','contexto'])
    # Aplicar la función de embedding a los datos
    product_data_df['text_embedding'] = product_data_df.contexto.apply(lambda x: get_embedding(x, engine='text-embedding-ada-002'))
    ...
    for index, row in product_data_df.iterrows():
        brand_dict = {'role': "assistant", "content": f"{row['url']}"}
        products_list.append(brand_dict)
    message_objects.extend(products_list)

    # Mensajes de introducción para el chat
    message_objects.append({"role": "system", "content": "Soy un chatbot de ventas de una empresa de alquiler de lanchas. si el cliente pide informacion acerca de que se puede hacer en una salida en lancha lee el articulo y pasale la informacion que hay en el"})
